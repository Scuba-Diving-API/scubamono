  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <a href="/diving/admin/disciplines/create" className="bg-orange-600 text-white py-2 px-4 rounded hover:bg-orange-700">
          Add New Discipline
        </a>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {disciplines.map((discipline) => (
              <tr key={discipline.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">{discipline.status}</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                  <a href={`/diving/admin/disciplines/edit/${discipline.id}`} className="text-indigo-600 hover:text-indigo-900">
                    Edit
                  </a>
                  <button onClick={() => handleDelete(discipline.id)} className="text-red-600 hover:text-red-900">
                    Delete
                  </button>
                </td>
              </tr>
