...
const categories = ["Accessories", "Certifikater", "DSF varer", "Logbøger og -blade", "Sikkerhedsudstyr og signalflag", "Tabeller", "Uddannelses- og lærebøger", "Ungdomsaktiviteter"];
      ...
      <div className="bg-blue-900 text-white">
        <div className="container mx-auto px-4 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative w-64">
                <input type="text" placeholder="Skriv søgeord" className="w-full px-4 py-2 bg-white text-gray-800 rounded-full focus:outline-none" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          ...
          <div className="mt-4 flex space-x-6">
            <Link to="/webshop" className="hover:text-cyan-300 font-medium">
              Alle produkter
            </Link>
            <Link to="/webshop/min-konto" className="hover:text-cyan-300 font-medium">
              Min Konto
            </Link>
            <Link to="/webshop/handelsbetingelser" className="hover:text-cyan-300 font-medium">
              Handelsbetingelser
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-1/4 bg-white rounded-lg shadow-md p-6">
            ...
            <ul className="space-y-2">
              <li>
                <button className={`w-full text-left py-2 px-3 rounded-md transition-colors ${activeCategory === null ? "bg-blue-900 text-white" : "hover:bg-gray-100"}`} onClick={() => setActiveCategory(null)}>
                  Alle kategorier
                </button>
              </li>
              {categories.map((category, index) => (
                <li key={index}>
                  <button className={`w-full text-left py-2 px-3 rounded-md transition-colors ${activeCategory === category ? "bg-blue-900 text-white" : "hover:bg-gray-100"}`} onClick={() => setActiveCategory(category)}>
                    {category}
                  </button>
                </li>
              ))}
          ...
          <div className="md:w-3/4">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">{activeCategory ? `Produkter i ${activeCategory}` : "Fremhævede produkter"}</h2>
                ...
                {filteredProducts.map((product) => (
                  <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden transition-all hover:shadow-lg">
                    <div className="h-48 overflow-hidden relative">
                      ...
                      {product.isOnSale && <div className="absolute top-0 left-0 bg-red-500 text-white text-xs px-3 py-1">UDSALG</div>}
                    </div>
                    <div className="p-4">
                      ...
                      <button onClick={addToCart} className="w-full bg-blue-900 text-white py-2 rounded-md hover:bg-blue-800 transition-colors">
                        Tilføj til kurv
                      </button>
                    </div>
                  </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <p className="text-gray-600">Besøg vores hovedhjemmeside for at finde flere informationer om Dansk Sportsdykker Forbund, dets aktiviteter og muligheder.</p>
